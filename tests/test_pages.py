...
class PageTests(TestCase):
    ...
    def test_home_and_project_pages(self):
        ...
        self.assertContains(meaning, "Track my county")
        self.assertContains(meaning, "Watch and compare")
        self.assertContains(meaning, "Features")

        method = self.client.get(reverse("core:how-it-works"))
        self.assertEqual(method.status_code, 200)
        self.assertContains(method, "How it works")
        self.assertContains(method, "Search → Project → Evidence → Investigation → Report")
        self.assertContains(method, "Track my county")
        self.assertContains(method, "Compare")
        self.assertContains(method, "Evidence coverage")

        features = self.client.get(reverse("core:features"))
        self.assertEqual(features.status_code, 200)
        self.assertContains(features, "Features")
        self.assertContains(features, "Track my county")
        self.assertContains(features, "Track and favourite a project")
        self.assertContains(features, "Compare projects and counties")
        self.assertContains(features, "Dashboard")
        self.assertContains(features, "Investigations")
        self.assertContains(features, "Reports")
        self.assertContains(home, "Features")
